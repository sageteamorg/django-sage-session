...
class TestSessionHandler:
    ...
    def test_session_expiration(self, factory, user):
        ...
        with patch('django.utils.timezone.now', return_value=timezone.now() + timezone.timedelta(minutes=2)):
    ...
    def test_handle_expiration(self, factory, user):
        ...
        with patch('django.utils.timezone.now', return_value=timezone.now() + timezone.timedelta(seconds=2)):
            with patch('django.contrib.messages.info'):  # Patch messages to avoid MessageMiddleware issues
