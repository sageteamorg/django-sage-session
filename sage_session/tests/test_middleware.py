...
class TestSessionManagementMiddleware:
    ...
    def test_session_expiration(self, factory, user):
        ...
        expired_session = UserSession.objects.create(
            ...
            device_info='Device',
            expires_at=timezone.now() - timezone.timedelta(minutes=1)  # Expired session
        )
    ...
    def test_max_sessions_reached(self, factory, user):
        ...
        for i in range(10):
            new_session = self.create_django_session(user)  # Create a truly unique session using new session keys
            UserSession.objects.create(
                user=user,
