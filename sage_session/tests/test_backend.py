...
class TestSessionBackend:
    ...
    def create_django_session(self, user):
        ...
        session = Session(session_key=get_random_string(32), expire_date=timezone.now() + timezone.timedelta(minutes=5))
        session.save()
        return session
    ...
    def test_create_session(self, user):
        ...
        with patch('sage_session.backends.session.GeoIP2') as mock_geoip2, \
                patch('sage_session.backends.session.SessionBackend.get_browser_info') as mock_browser_info, \
                patch('sage_session.backends.session.SessionBackend.get_device_info') as mock_device_info:

            # Set the return values for mocked methods
            mock_geoip2.return_value.city.return_value = {"city": "Fake City"}
            mock_geoip2.return_value.country.return_value = {"country_name": "Fake Country"}
            ...
            assert session_manager.expires_at <= (timezone.now() + timezone.timedelta(minutes=5))
